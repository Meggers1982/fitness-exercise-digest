...
Your readers pitch to publications like Runner's World, Outside Magazine, Men's Health, Women's Health Magazine, Shape, Self, Well+Good, and science-forward outlets covering fitness and active living.
...
{{
  ...
  "groundbreaking": "one or more of: Counterintuitive finding / Overturns prior research / First-in-class human study / Clear fitness or performance implication",
  "media_coverage": "{media_note}",
  "summary": "2-4 sentences: what researchers did, who participated (N=X, fitness level or athletic population if noted), key finding in plain language — verified against abstract",
  "why_it_matters": "1-2 sentences of real-world significance for people who exercise or want to. Do NOT imply clinical action.",
  "caveats": "comma-separated flags: small sample (N<100), observational design, single-center, self-reported outcomes, short follow-up, industry funding [name], preprint, secondary analysis, elite athlete sample (may not generalize), animal or cell study — or 'None identified'",
  "fact_check_note": "Describe any corrections made vs what the abstract actually says, or empty string",
  "excluded": false,
  "relevance_score": 7,
  "relevance_score_reason": "One sentence explaining the score — what makes it strong or weak for fitness and exercise journalism",
  "pitch_angles": [
    {{
      "publication_type": "e.g. Runner's World / Outside Magazine / Men's Health / Women's Health / Shape / Self / Well+Good / General fitness",
      ...
      "pitch_angle": "2-3 sentence commissioning pitch: what happened, why it matters for exercisers, practical or performance hook",
...
- Generate MULTIPLE angles (2-3 max) only when the study genuinely fits different audiences with meaningfully different framings — e.g. a strength training study could pitch differently to Men's Health (performance/muscle angle) vs Well+Good (general wellness angle) vs Outside Magazine (endurance/outdoor angle)
- Do not pad with extra angles if one covers it

Rules for content:
- Verify sample size (N), study design, direction of effect against the abstract
- Note whether the study population is elite athletes, recreationally active adults, or sedentary individuals — this dramatically affects generalizability
- For sports medicine: distinguish injury prevention, treatment, and return-to-sport outcomes — do not conflate
...
relevance_score rubric (1–10): start at 5, then adjust:
  +2 counterintuitive or overturns prior belief about exercise, recovery, or nutrition
  +2 human subjects with recreationally active or general-population sample (N≥100)
  +1 clear practical takeaway for everyday exercisers
  +1 clean design (RCT, longitudinal, large cohort)
  +1 directly addresses workout programming, injury prevention, recovery, body composition, or exercise motivation
  −1 per major caveat
  −1 elite athlete sample only (limited generalizability)
...
{studies_block}"""
    ...
    try:
        ...
        return [{"pmid": s["pmid"], "headline": s["title"], "journal": s["journal"],
                 ...
                 "groundbreaking": "Clear fitness or performance implication", "media_coverage": media_note,
...
def send_notification(category: str, chunk_label: str, study_count: int, run_date: str):
    subject = f"Fitness & Exercise Research Digest — {run_date} | {study_count} {'Study' if study_count == 1 else 'Studies'}"
    html = f"""<!DOCTYPE html>
<html>
<body style="font-family:Georgia,serif;max-width:500px;margin:auto;padding:24px;color:#222;">
<h2 style="color:#1a3a2e;">Fitness &amp; Exercise Research Digest</h2>
...
<p style="font-size:0.85em;color:#888;margin-top:2em;">
  Fitness &amp; Exercise Research Digest · PubMed + Claude + SERPAPI
</p>
</body>
</html>"""
    try:
...
def main():
    ...
    print("=" * 60)
    print("Fitness & Exercise Research Digest")
